...
from calculate_BMI_second_approach import cal_bmi
from calculate_BMI_second_approach import get_category_and_healthrisk
from calculate_BMI_second_approach import generate_bmi_data
from calculate_BMI_second_approach import get_counts_from_ranges
...
def df_res():
    ...
    df_res = df.apply(generate_bmi_data, axis=1)
...
def test_get_category_and_risk(bmi_value, result_category, result_risk):
    ...
    assert get_category_and_healthrisk(bmi_value) == (result_category, result_risk)

def test_generate_bmi_data_columns(df_input):
    """Assert that for input data frame the correct columns (all three : BMI value, BMI Category, Health risk) are added."""


    df_input = df_input.apply(generate_bmi_data, axis=1)
...
def test_generate__bmi_data_values(input_dict_or_series, result_dict_or_series):
    """Assert that for whole data Frame correct values are calculated for all the three columns/fields."""

    result_dict = generate_bmi_data(input_dict_or_series)
